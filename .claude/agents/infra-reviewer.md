...
3. **Network is closed by default.** Data services (Postgres, Redis) bind to **loopback / the internal Compose network only** — never published to the host's public interface. The VM **NSG denies all inbound**; ingress is the **Cloudflare Tunnel** (cloudflared dials outbound); the public IP is egress-only.
