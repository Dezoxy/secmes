...
**Reading the table:** the four invariants that carry the E2EE promise (1, 3, 4, 6) are clean PASS. Invariant 2 is **PARTIAL** — its logging clause is PROVEN, but its persistence clause is violated by one bounded, host-root-gated case (INF-4, the cloudflared tunnel token in container metadata), so it does not get a clean pass until that token moves to a credentials file. Invariant 5 is PASS on its delivery rule with rotation residuals (the long-lived Arc/B2 credentials an operator must own). The visible weaknesses below live in **availability** (BKP-1), **active-attacker client exfil** (CDI-1 + CSP-1), **device-linking MITM** (FP-1), **at-rest retention** (F1/AR-1), and the **one secret-persistence residual** (INF-4) — not in the confidentiality core.
