...
- Verify `TextEncoder`/`TextDecoder` — including the strict `new TextDecoder('utf-8', { fatal: true })` in `index.ts` (a security control against lossy-UTF-8 identity collisions). **Gate it on a malformed-UTF-8 negative test that must *throw*.** If Hermes's `fatal` mode is absent, use a polyfill that genuinely implements `fatal: true` (e.g. `text-encoding` / `@sinonjs/text-encoding`). **Do NOT use `fast-text-encoding`** — it ignores `fatal` and decodes malformed input with replacement characters, silently removing the guard.
