...
| 9 | **Native refresh token is sender-constrained (DPoP-style), NOT origin-classified** | `security-architect` (2026-06-26): iOS native passkeys assert the same `https://<rpID>` origin as the PWA, so origin can't tell native from web. Bind the refresh token to a dedicated non-exportable, biometric-gated keystore signing key (own `argus-refresh-pop:v1` domain); every rotation verifies a hardware-key proof-of-possession, so a leaked body token is inert. Web keeps the HttpOnly cookie unchanged. Keeps attestation deferred. See [03](./03-roadmap-ios-then-android.md) Phase 1 + [04](./04-security-and-threat-model.md) §6. |
