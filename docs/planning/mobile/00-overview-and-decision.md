...
**The fix** (Phase 0, crypto-reviewer-gated): author a custom `ts-mls` `CryptoProvider` whose KEM is backed by `@noble/curves` x25519, and reroute the four `crypto.subtle` sites in your own code (`index.ts` SHA-256, `seal.ts` AES-GCM, `turn-credential.ts` HMAC) through `@noble`. This makes `@argus/crypto` **truly platform-pure** — it then runs identically on web and Hermes, which *also* benefits the PWA.
