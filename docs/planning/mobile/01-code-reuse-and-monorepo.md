...
| `lib/api-client.ts` + `lib/api.ts` (~60 endpoints) | **Medium** | `fetch`/`Headers` exist on RN, but the **HttpOnly refresh-cookie** model does not (no shared cookie jar). Move to the **sender-constrained** native refresh-token contract ([03](./03-roadmap-ios-then-android.md) Phase 1, decision #9): the server returns the rotated token **in-body** for native; the client stores it in the OS keystore and **signs each refresh with the hardware PoP key** (so a leaked token is inert). The `Fetcher` is already injectable. |
