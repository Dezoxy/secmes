...
`apps/api` and `infra/*` are **unchanged** by the client pivot, except for the two additive server deliverables in [03](./03-roadmap-ios-then-android.md) Phase 1 (origin allowlist, native push-token contract). One language, one contracts source, one crypto engine across `api` + `web` + `mobile`.
