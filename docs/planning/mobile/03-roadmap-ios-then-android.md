...
- **Native push-token contract + sender.** Add an APNs/FCM device-token registration shape to `@argus/contracts` (replacing the Web-Push/VAPID `SubscribePushRequest` for native). Add a `push_subscriptions`-style table via the `/db-migration` skill: `tenant_id` + `ENABLE`+`FORCE` RLS `TO argus_app` + leading `tenant_id` index + caller-owns-device check. Build the server-side push sender (none exists today) emitting **content-free** payloads: message wakes carry a **fixed generic visible alert** (`alert: "New message"`) alongside `{type:'new_message'}` — so the OS notifies even if iOS drops the background JS wake (see [04](./04-security-and-threat-model.md) §3); call wakes use the native-call module's documented **`incomingCall` shape** via PushKit / FCM-high-priority (`serverCallId`/`eventId` = the opaque `callId`, FCM `messageType:'incomingCall'`, **generic** caller field only — a bare `{type:'call', callId}` won't be parsed before JS starts and the locked ring fails; custom native parsing is the alternative). **No** `conversationId`, `senderId`, text, or caller identity in any payload — the fixed string carries no metadata. APNs `.p8` / FCM service-account delivered as **Key-Vault file secrets** via Managed Identity (never env), mirroring `VAPID_PRIVATE_KEY_FILE`. Never log tokens.
- **Native refresh-token transport contract (decision #9).** The 30-day refresh token is delivered today only as an **HttpOnly cookie** scoped to `/api/auth/session/refresh` (web; `X-Argus-Refresh` is a CSRF marker). The native fix **preserves HttpOnly end-to-end**: native HTTP stacks **have a cookie jar** (iOS `NSURLSession`/`NSHTTPCookieStorage`; Android OkHttp with a persistent `CookieJar`) that transparently stores and **resends** the path-scoped cookie — the app needs the platform to *resend* it, not to *read* it — so the token is never exposed to JS on any platform (this corrects the earlier "native has no cookie jar" premise). **Web-origin sessions — and the same-origin iOS associated-domain case (its passkey origin is byte-identical to the PWA's `https://<rpID>`, `webauthn.service.ts:80`) — are ALWAYS cookie-only, never body-delivered**, so web XSS cannot elect a body refresh token and gain a persistent session. The cookie jar also resends in the **background with no user present**, so a locked-phone call wake can refresh. **Fallback** (only where the platform cookie jar proves unreliable, and **only** for a provably-distinct native origin like Android `apk-key-hash` or behind an unspoofable native signal — never a same-origin client): a **sender-constrained (DPoP-style) body token** bound to a **non-exportable, *background-signable* (device-unlock-class, NOT per-use-biometric, so it signs during locked-call wakes) hardware key** with its own `argus-refresh-pop:v1` domain; each `POST /auth/session/refresh` verifies a fresh PoP over the single-use token-hash before rotating, so a leaked body token is inert. Rotation + refresh-reuse family-revocation + the rotation-race lock are preserved on every path; crypto-blind holds. **The final mechanism (cookie-jar vs sender-constrained body) and platform cookie-persistence + CSRF validation are settled in the Phase-1 `security-architect` pass before code.** Add the shape to `@argus/contracts`; pin route posture in a controller spec; refresh `openapi.json` + 42Crunch; threat-model note `docs/threat-models/native-refresh-pop.md` before code.
...
**Verification:** `security-boundary-auditor` (RLS, no token logging, no IDs in payload, exact-allowlist origins, **and that web-origin (and same-origin iOS) sessions are cookie-only — never body-delivered**; native refresh keeps the token out of JS via the platform cookie jar; any body-delivery fallback is **sender-constrained** (fresh hardware-key PoP over the single-use token-hash, domain `argus-refresh-pop:v1`, total/never-throws → opaque 401, **before** rotation) on a **background-signable, non-exportable** key (signs during locked-call wakes; **not** the heap-resident MLS key), granted only to a provably-distinct native origin / unspoofable signal; rotation + reuse-detection + family-revocation preserved; a missing/invalid PoP is a 401 with **no rotation and no family-revocation**; no token/key/proof bytes are logged); db-migration RLS verified; `infra-reviewer` on the Key-Vault file secret; 42Crunch 90+; controller specs green; a manual register/authenticate round-trip from a native-shaped origin verifies against the widened pin.
...
- **Token storage.** Consume the Phase-1 refresh contract (decision #9): the **primary path is the platform cookie jar** — configure `NSHTTPCookieStorage` (iOS) / a persistent OkHttp `CookieJar` (Android) so the HttpOnly, path-scoped refresh cookie is stored and resent transparently, **without the app reading the token** (it never enters JS); send the `X-Argus-Refresh` CSRF header on refresh. On iOS this is the **only** path (same origin as web ⇒ always cookie-only). The sender-constrained **body token is the fallback only** (a provably-distinct Android `apk-key-hash` origin where the cookie jar proves unreliable) — stored in the OS keystore and signed with the background-signable PoP key. The `Fetcher` seam is already injectable; access tokens stay in memory.
...
- **Locked-phone decryption boundary (V1 = unlock-before-media — decision #10).** The keystore root is biometric-gated and working keys are wiped on lock/background (decision #8), so a locked phone can **ring** (CallKit, **generic label — no caller identity on the lock screen**, matching the content-free push) but **cannot decrypt the MLS-sealed `CallSignal`/SDP while locked**. V1: **answering** brings the device to unlock (Face ID/passcode) → the keystore unlocks → the `CallSignal` decrypts → media connects. The lock-screen **ring** is the capability the PWA fundamentally lacks; **connecting audio requires user presence** (which is natural — the user is picking up the call). A **background-accessible call-signaling key tier** (after-first-unlock, non-biometric, separate from the at-rest root) that would allow seamless lock-screen answer is a **deferred option with a documented at-rest tradeoff**, decided in the Phase-4 `security-architect` pass / `native-keystore-unlock.md`.
- Wire VoIP entitlement / background modes (`voip`, `remote-notification`). The server push sender emits the call wake in the **chosen native-call module's documented incoming-call shape** so the native PushKit/FCM handler reports the call *before JS starts* — for `expo-callkit-telecom` that's an `incomingCall` wrapper (`serverCallId`/`eventId` = the opaque server-minted `callId`; FCM `messageType:'incomingCall'`) with a **generic** caller field only (e.g. "Argus call"); a bare `{type:'call', callId}` would be parsed as a normal push and the locked-phone ring would fail. A custom native PushKit/FCM parser is the alternative. **Content-free**: no real caller identity in the push (resolved after decrypt — decision #10).
...
- PushKit VoIP push — the only API that legitimately triggers CallKit on a locked device; `reportNewIncomingCall` synchronously per push; payload in the native-call module's `incomingCall` shape (opaque `serverCallId`/`eventId` = `callId`, **generic** caller field) so it's parsed before JS starts — content-free (no real caller identity).
