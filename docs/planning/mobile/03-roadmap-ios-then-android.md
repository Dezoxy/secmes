...
- **Native refresh-token transport contract (sender-constrained — decision #9).** The rotated 30-day refresh token is delivered today only as an **HttpOnly cookie** scoped to `/api/auth/session/refresh` (`X-Argus-Refresh` is just a CSRF marker; the token is read from `req.cookies`), unreadable by a native client with no cookie jar — so native refresh is impossible as-is. **The web-vs-native delivery split is *not* a security boundary**: the iOS associated-domain passkey origin is byte-identical to the PWA's `https://<rpID>` (`webauthn.service.ts:80`), so `clientDataJSON.origin` can't distinguish them, and a client-claimed flag is spoofable. Instead make the token **sender-constrained** (DPoP-style): at register/authenticate the native client registers a **dedicated non-exportable, biometric-gated hardware-keystore signing key** — the decision-#8 keystore, a *separate* handle from the heap-resident MLS Ed25519 key, with its own `argus-refresh-pop:v1` domain — whose **public** key is recorded on the session/refresh-token family alongside a `delivery` preference. Native families get the rotated token **in the response body**; web families are **unchanged** (HttpOnly cookie, `pop_public_key NULL`). Every `POST /auth/session/refresh` on a PoP-bound family must carry a fresh detached signature over the **single-use current-token-hash**, verified **before** rotation (total/never-throws → opaque 401) — so a leaked/XSS-grabbed body token is **inert** without the hardware key, and body-vs-cookie stops being a boundary. Rotation + refresh-reuse family-revocation + the rotation-race lock are preserved; the PoP public key copies across each rotation; a missing/invalid PoP is a **401 with no rotation and no family-revocation** (auth failure, not theft). Crypto-blind holds (Ed25519 auth over a public key, never decryption). Add the shape to `@argus/contracts`; pin the route posture in a controller spec; refresh `openapi.json` + 42Crunch. New threat-model note `docs/threat-models/native-refresh-pop.md` before code.
...
**Verification:** `security-boundary-auditor` (RLS, no token logging, no IDs in payload, exact-allowlist origins, **and that in-body refresh-token delivery is *sender-constrained*, not origin-classified**: every `POST /auth/session/refresh` on a family with a recorded PoP public key verifies a fresh hardware-key proof-of-possession (domain `argus-refresh-pop:v1`, bound to the single-use token-hash, total/never-throws → opaque 401) **before** rotation; the PoP key is the device's **non-exportable biometric-gated keystore** key (decision #8), **not** the heap-resident MLS Ed25519 key; the PoP public key propagates across rotation; web families stay `pop_public_key IS NULL` + HttpOnly-cookie unchanged; a missing/invalid PoP is a 401 with **no rotation and no family-revocation**; no token/key/proof bytes are logged); db-migration RLS verified; `infra-reviewer` on the Key-Vault file secret; 42Crunch 90+; controller specs green; a manual register/authenticate round-trip from a native-shaped origin verifies against the widened pin.
