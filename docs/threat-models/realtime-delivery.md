...
- **Delivery-gap detection — implemented, observability-only (Track 3 item D, PR 3b).** Each `message` frame now carries a per-`(socket, conversation)` transport counter (`deliverySeq`, with `deliveryPrevSeq` for the prior frame / `null` on the first). The gateway stamps it **EPHEMERALLY at fan-out** (a `Map` on the per-socket connection state, dropped on disconnect/removal); it is **never persisted**, adds no DB column/table, and resets on every (re)subscribe. The client tracks the last value per conversation and, on a non-contiguous frame, re-fetches over the **existing** `(created_at, id)` catch-up path (dedup by `id`) — turning a previously *silent* dropped/reordered live frame into a self-healing backfill. **During that catch-up the client defers inline decryption of the conversation's live frames** so the backfill decrypts strictly in MLS generation order from the durable cursor; this avoids consuming a later generation before the missed earlier one, whose skipped key MLS only caches for a bounded window (`retainKeysForGenerations`), which a burst could otherwise evict. It is **metadata only and carries no cryptographic guarantee**: a gap merely *triggers* a fetch — the counter never gates decryption, ordering, or dedup (MLS owns content integrity; the DB + keyset cursor own ordering/no-loss), is distinct from the MLS `epoch` and ratchet `generation`, and lives **outside** the ciphertext envelope. There is **no server-consumed ACK** — the client acts locally, so this path can never advance a delivery/retention watermark (keeps the Track 4 per-user-vs-per-device deletion boundary intact). **Truncation / tail-withholding stays an accepted relay residual** (next item): a counter cannot distinguish "no newer messages" from a dishonest server stopping at *N*. The persistent commit-order watermark below remains the deferred enterprise alternative.
