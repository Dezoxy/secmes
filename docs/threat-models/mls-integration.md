...
1. **Hand-rolled crypto / primitive misuse (Tampering/Info-disclosure).** → **All MLS message crypto** goes through ts-mls; the wrapper hand-rolls **no MLS primitives** (no AEAD/KDF/curve code, no key derivation for the protocol). The only non-MLS primitives in `packages/crypto` are two deliberate, crypto-reviewer-cleared exceptions: **at-rest AES-GCM sealing** under the WebAuthn-PRF unlock key (`seal.ts`, `prf-keystore-unlock.md`) and **Ed25519 device-proof** signing/verification for multi-device enrollment (`device-proof.ts`) — both WebCrypto/`@noble`, neither touches MLS message confidentiality. Randomness comes from WebCrypto CSPRNG; **no `Math.random`** in this package (Semgrep-enforced).
