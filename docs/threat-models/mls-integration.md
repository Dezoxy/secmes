...
7. **Spent ratchet secrets retained (forward secrecy).** ts-mls returns `consumed` (spent secrets) from each op; the wrapper discards them and never copies them into long-lived structures. JS cannot guarantee memory zeroization, so true erasure is best-effort — a known, accepted limitation of a JS/WASM client.
