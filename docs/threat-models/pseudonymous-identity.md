...
`email`. Migration `0016` NULLs **every** existing `display_name` (removing all legacy IdP real names AND, since
NULLs are distinct, letting the unique index build even where legacy names duplicated), then adds the index;
each user is healed to a fresh handle on next login (`coalesce(display_name, <new handle>)`). **Gated by:**
`security-boundary-auditor` (provisioning + migration + RLS), Semgrep (`argus-no-insecure-random`), and the
service tests (uniqueness, conflict-retry, existing-user-keeps-handle, NULL-handle-healing).
...
- **Legacy display names** — migration 0016 resets EVERY existing `display_name` to NULL, so no legacy IdP real
  name persists; each pre-#44b user shows a NULL handle (the client treats `display_name` as nullable) until they
  next log in, at which point provisioning heals it to a generated handle. The only residue is a *blank* display
  for an inactive account until its owner next authenticates — never a real name. No production data exists yet
  (local-first build).
