...
- **In-window token replay / no pre-expiry JWT revocation** — the access token is a stateless JWT with no `jti`/denylist. A denylist is **deliberately not built**: it would add a stateful per-request lookup for a residual already bounded by a short TTL + the active-user re-check below (revisit only alongside the DPoP/introspection line). Precisely what this residual does and does NOT cover:
  ...
  - **Mitigation that must be real:** pin a **short access-token lifetime (~10–15 min)** in Zitadel, leaning on the in-memory `offline_access` refresh token for silent renewal (§8). Zitadel's default is **12h** — too long for the "short TTL mitigates replay" argument. The access-token lifetime is an **instance-level OIDC setting** (Admin API `PUT /admin/v1/oidc/settings`), not the per-app config `infra/local/zitadel/provision.sh` creates. **Operational requirement (prod): production Zitadel MUST pin this TTL** — that instance setting, not anything in this repo, is what actually bounds residual (a). `provision.sh` carries a comment documenting the same for local parity.
...
> **Status: IMPLEMENTED.** Wired in `apps/web/src/lib/auth.ts` (the `oidc-client-ts` `UserManager`), `routes/Callback.tsx` (the code→token exchange), `features/auth/AuthContext.tsx` (session state + the `RequireAuth` gate), and `lib/api.ts` (the Bearer-attached client + `establishSession`). The login config (authority, client_id, redirect_uri, PKCE) is verified against the running Zitadel, and the API edge (§1) validating these exact tokens was proven end-to-end (real login → `/me` 200). The controls below are now enforced, not aspirational.
