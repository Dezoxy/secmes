...
- **In-window token replay / no pre-expiry JWT revocation** — the access token is a stateless JWT with no `jti`/denylist. A denylist is **deliberately not built**: it would add a stateful per-request lookup for a residual already bounded by a short TTL + the active-user re-check below (revisit only alongside the DPoP/introspection line). Precisely what this residual does and does NOT cover:
  - **Account-delete is neutralized on the next request — not residual.** `gdpr.deleteAccount` removes the user + `user_tenant_index` row (→ unbound → 403), so a still-valid bearer token cannot act. **Member-revoke is only *partially* neutralized** — paths that resolve the caller via `requireUser` (`messaging/membership.ts`, `status='active'`) and `admin.guard.ts` reject it immediately, but the key-directory mutation routes resolve the caller without a `status` check, leaving a ≤10-min window. **See the corrected analysis in `session-tokens.md` §ST-1** (this earlier "neutralized on *every* path" wording was too broad).
