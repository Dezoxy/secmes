...
- **R-friends-3 (request-create enumeration oracle):** `POST /friends/requests` is a state-changing argus-id probe. Mitigated by a **uniform `202` with a constant body** (`{ status: 'accepted' }` for found / not-found / inactive / self / already-friends / already-pending — verified by the service test) + the dedicated `SENSITIVE_LIMITS.sendFriendRequest` = 10/hour limit. The target argus-id is **log-sanitised** (`ARGUS_ID_RE`, else `<invalid-format>`) before the `friends.request_created` audit row, reusing the lookup pattern.
  - **Residual (accepted):** a created pending request is visible to the *requester* immediately via `GET /friends/requests?box=outgoing` (and their own GDPR export), so a sender who already holds a candidate argus-id can confirm it resolved to an active user *before* the recipient consents — the row exists only for a real, active, not-already-connected target. This is **equivalent to the existing, by-design `users.lookup` oracle** (`GET /users/lookup` already returns found/inactive-vs-404 for an exact argus-id at a *looser* 10/min cap): both require the attacker to already possess the full ~80-bit argus-id, and **neither enables enumeration** of the id space (exact-match only, no prefix/fuzzy). The uniform `202` still prevents the send path from becoming a *distinct, higher-rate* oracle and prevents any who-declined ledger. **Fully removing the pre-consent signal requires the deliver-only model** (no `pending` row persisted until accept), explicitly deferred under R-friends-2 as more than the beta needs.
