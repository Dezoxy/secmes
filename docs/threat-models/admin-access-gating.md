...
**Scope:** moving the breakglass ADMIN login off the public landing page onto a dedicated path (`/admin`) and
gating that path **plus the admin/breakglass API** (`/api/auth/breakglass/*`, `/api/admin/*`) behind
**Cloudflare Access**, so the highest-privilege credential is not exposed to the open internet. Companion to
[`breakglass-admin.md`](breakglass-admin.md) and [`vm-ingress.md`](vm-ingress.md).
...
- **Layer A — edge (Caddy, the load-bearing control):** `infra/stack/caddy/Caddyfile` returns **404** for
  `/admin`, `/admin/*`, `/api/auth/breakglass/*`, `/api/admin/*` when the `Cf-Access-Jwt-Assertion` header is
  absent. cloudflared injects that header only on requests that passed the Access policy, and strips any
  client-supplied copy. 404 (not 403) so the routes are indistinguishable from non-existent to an
  unauthenticated scanner. The block is first among the `handle` blocks, so it wins before the API proxy and
  the SPA fallback. The public app, passkey login, and end-user `/api` routes are untouched.
- **Layer B — app (defense in depth, env-gated):** `CfAccessGuard` on `BreakglassController` +
  `AdminController` verifies the `Cf-Access-Jwt-Assertion` signature with `jose` against the team JWKS
...
| T4 | **Forged `Cf-Access-Jwt-Assertion` header.** A request inside the origin network spoofs the header to bypass Access. | Layer A trusts cloudflared header hygiene (it strips client copies; the VM has no inbound ports and only the internal Docker network reaches `caddy:8080` — `vm-ingress.md`). Layer B closes it in prod: `CfAccessGuard` verifies the JWT **signature** + iss/aud/exp, so a forged header without a valid team-signed JWT is rejected. |
