  ...
  return (
    <div className="flex items-center justify-between gap-4 rounded-xl border border-white/5 bg-white/[0.03] px-4 py-3">
      <div>
        ...
        <p className="mt-0.5 text-xs text-white/40">{value}</p>
      </div>
      {badge ? (
        ...
        <span
          className={`h-6 w-10 rounded-full border p-0.5 transition-colors ${
            disabled ? 'border-white/10 bg-white/5' : 'border-purple-400/40 bg-purple-500/30'
          }`}
        >
          <span
...
function SectionPlaceholder({ icon: Icon, title }: { icon: LucideIcon; title: string }) {
  return (
...
export function SettingsPanel({ profile, deviceId, onProfileChange, onClose }: SettingsPanelProps) {
  ...
  return (
    ...
    >
      <div className="flex h-[90vh] w-full max-w-4xl overflow-hidden rounded-2xl border border-white/5 bg-[#12121a] shadow-2xl shadow-black/50 sm:h-[82vh]">
        <aside
          className={`${
        ...
        >
          <div className="mb-6 flex items-center gap-3">
            <button
              ...
              className="rounded-lg p-2 text-white/50 transition-colors hover:bg-white/5 hover:text-white sm:hidden"
            ...
            <button
              type="button"
              onClick={onClose}
              className="ml-auto rounded-lg p-2 text-white/50 transition-colors hover:bg-white/5 hover:text-white sm:hidden"
              aria-label="Close settings"
            >
              <X className="h-5 w-5" />
            </button>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => void copyId()} className={SUBTLE_BUTTON}>
          ...
          {active === 'security' && (
            <div className="space-y-3">
              <SettingRow
                title="Passkey-only login"
                value="Managed by Zitadel policy"
                disabled={false}
              />
          ...
          {active === 'privacy' && (
            <div className="space-y-3">
              <SettingRow title="Read receipts" value="Uses the product default" badge="Default" />
              <SettingRow
                title="Typing indicators"
                value="Uses the product default"
                badge="Default"
              />
              <SettingRow title="Link previews" value="Uses the product default" badge="Default" />
            </div>
          )}
          ...
          {active === 'notifications' && (
            <div className="space-y-3">
              <SettingRow
                ...
                badge="Auto"
              />
              <SettingRow title="Mentions only" value="Uses the product default" badge="Default" />
              <SettingRow title="Quiet hours" value="Uses the product default" badge="Default" />
              <SectionPlaceholder icon={Bell} title="Conversation mute controls" />
            </div>
          )}
          ...
          {active === 'storage' && (
            <div className="space-y-3">
              <SettingRow
                ...
                disabled={false}
              />
              <SettingRow title="Clear local cache" value="Needs confirmation flow" />
              <SettingRow title="Media auto-download" value="Off until attachment backend lands" />
            </div>
          )}
          ...
          {active === 'devices' && (
            <div className="space-y-3">
              ...
              <SettingRow title="Trusted devices" value="Requires backend device registry UI" />
              <SettingRow title="Revoke device" value="Requires backend revoke endpoint" />
            </div>
          )}

          {active === 'about' && (
            <div className="space-y-3">
              <SettingRow title="App" value="Argus secure messaging" disabled={false} />
              <SettingRow title="Backend status" value="Diagnostics menu reserved" />
              <SectionPlaceholder icon={Monitor} title="Safe diagnostic export" />
            </div>
          )}
        </main>
      </div>
