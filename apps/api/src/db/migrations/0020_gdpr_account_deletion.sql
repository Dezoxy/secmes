...
-- 2. Grant UPDATE on sender_user_id to argus_app.
...
grant update (sender_user_id) on messages to argus_app;
...
-- Grant UPDATE on created_by to argus_app so the erasure path can null it out.
-- Column-level grant limits scope: only created_by can be updated, not other columns.
grant update (created_by) on conversations to argus_app;
